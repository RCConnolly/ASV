 ...
 * Returns: none
*/
Coord utm2image(Point & utm_pt, const int bottom, const int left, const int width, const int height) {
    
    assert(utm_pt[0] > bottom);
    assert(utm_pt[1] > left);

    unsigned int y_img = utm_pt[0] - bottom;
    unsigned int x_img = utm_pt[1] - left;
...
*/
void computePath(Path & path, const Coord & start, const Coord & goal, const std::string filename) {
    // Loading grid.
    FMGrid grid_fm2;
    ...
    s->setInitialAndGoalPoints(start, goal);
    s->compute();
...
/* Prints each path point to std::out, with coordinates being in Lat/Long
void printPath(Path * path, const char * outfile) {
...
int main(int argc, const char ** argv)
{
    ...
    const int bottom = 3836014;
    const int left   = 340154;
    ...
    Coord start_coord = utm2image(start, bottom, left, width, height);
    Coord goal_coord  = utm2image(goal,  bottom, left, width, height);
    //std::cout << "start: {" << start_coord[0] << "," << start_coord[1] << "}\n";
    //std::cout << "goal: {" << goal_coord[0] << "," << goal_coord[1] << "}\n";

    // Compute path
    Path path;
    computePath(path, start_coord, goal_coord, image_name);

    // Print path to a file
    // printPath(path_ptr, "path.txt");
